...
import { PBLanguage, PBMetaPointer, PBNode } from "./Chunk.js";
...
function createBasePBBulkImport(): PBBulkImport {
  return { internedStrings: [], internedMetaPointers: [], internedLanguages: [], attachPoints: [], nodes: [] };
}
...
export const PBBulkImport = {
  encode(message: PBBulkImport, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    for (const v of message.internedLanguages) {
      PBLanguage.encode(v!, writer.uint32(42).fork()).ldelim();
    }
...
export const PBAttachPoint = {
  encode(message: PBAttachPoint, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.container !== 0) {
      writer.uint32(8).uint32(message.container);
    }
    if (message.metaPointerIndex !== 0) {
      writer.uint32(16).uint32(message.metaPointerIndex);
    }
    if (message.rootId !== 0) {
      writer.uint32(24).uint32(message.rootId);
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1:
          ...
          message.container = reader.uint32();
          continue;
        case 2:
          if (tag !== 16) {
            break;
